...
gnome-terminal -- bash -c "cd $BASE_DIR/cpu && ./cpu cpu1; exec bash"
...
curl -X POST -H "Content-Type: application/json" \
     -d '{ "pid": 1, "pc": 0 }' http://127.0.0.1:8081/recibirProceso

# Simular envío de petición a IO
echo "Enviando petición a IO..."
curl -X POST -H "Content-Type: application/json" \
     -d '{ "pid": 1, "tiempo": 3000 }' http://127.0.0.1:8080/recibirPeticion

# Simular mensaje a memoria desde CPU
echo "Petición CPU a Memoria..."
curl -X POST http://127.0.0.1:8082/siguienteInstruccion

# Simular mensaje a memoria desde Kernel
echo "Petición Kernel a Memoria..."
curl -X POST http://127.0.0.1:8082/iniciarProceso
